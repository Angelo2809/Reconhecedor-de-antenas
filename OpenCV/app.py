...
while True:
    ...
    contornos, hier = cv2.findContours(img_process, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(img_gray, contornos, -1 , (0,255,0), 3)
    ...
    for c in contornos:
        ...
        if perimeter > 150:
            ...
            print (f'{y} <- y, {lar} <- lar')
            try:
                if deltay / deltax > 0 and (deltax > 0 or deltay > 0):
    ...
    cv2.imshow("contorno", img_gray) 
    print(f'Foi localizado {count} antenas')
    if cv2.waitKey(0) == 27:
        break
